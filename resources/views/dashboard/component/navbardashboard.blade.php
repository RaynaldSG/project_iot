<nav class="navbar navbar-expand-lg bg-body-tertiary"
    ...
    <div class="container-fluid">
        ...
        <div class="justify-content-end d-flex" style="width: 25%" id="navbarSupportedContent">
            <a href="/dashboard/profile"><img class="img-fluid" style="height: 40px; border-radius: 50%;" src="@if (auth()->user()->image)
                {{ asset('storage/' . auth()->user()->image)  }}
            @else
                \assets\img\dashboard\profileNav.png
            @endif" alt="{{ auth()->user()->name }}"></a>
        </div>
    </div>
</nav>
